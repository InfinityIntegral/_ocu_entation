...
void SGDMSgmlGeneration::generateSgml(){
    ...
    if(SGXFileSystem::folderExists(SGDMDocumentationParsing::sourcePath + "SGML_source") == true){SGXFileSystem::permanentDeleteFolder(SGDMDocumentationParsing::sourcePath + "SGML_source");}
    SGXFileSystem::createFolder(SGDMDocumentationParsing::sourcePath + "SGML_source");
    SGXFileSystem::createFolder(SGDMDocumentationParsing::sourcePath + "SGML_source/cppclass");
    SGXFileSystem::createFolder(SGDMDocumentationParsing::sourcePath + "SGML_source/funclist");
    SGXFileSystem::createFolder(SGDMDocumentationParsing::sourcePath + "SGML_source/modules");
...
void SGDMSgmlGeneration::generateForNextClass(){
    ...
    if(currentClass.className.length() > 14 && currentClass.className.substringLeft(14) == "@SG_OTHERFILE_"){
        ...
        const int spaceIndex = currentClass.className.findFirstFromLeft(SGXChar(' '));
        const SGXString filePath = SGXFileSystem::joinFilePaths(SGDMDocumentationParsing::sourcePath + "SGML_source", currentClass.className.substring(14, spaceIndex - 14) + ".sgml");
        const SGXString pageTitle = currentClass.className.substringRight(currentClass.className.length() - spaceIndex - 1);
        ...
        fileContents += SGXString("@SG_ML_PAGENAME ") + pageTitle + "\n";
        fileContents += SGXString("@SG_ML_TITLE ") + pageTitle + "\n";
        fileContents += "@SG_ML_BLANKLINE\n";
        for(int i=0; i<currentClass.members.at("").description.length(); i++){
            fileContents += currentClass.members.at("").description.at(i) + "\n";
        }
        SGXFileSystem::createFile(filePath);
        SGXFile::writeAllText(filePath, fileContents.replace(SGXChar('<'), SGXChar(0x2039)).replace(SGXChar('>'), SGXChar(0x203A)));
        SGXTimer::singleCall(0.0f, &SGDMSgmlGeneration::generateForNextClass);
        return;
    }
    if(currentClass.className.length() > 11 && currentClass.className.substringLeft(11) == "@SG_MODULE "){
        SGXString fileContents = "";
        const SGXString moduleName = currentClass.className.substringRight(currentClass.className.length() - 11);
        fileContents += "@SG_ML_COMMENT This SGML documentation file is automatically generated by _ocu_mentation, the 05524F internal documentation generation app.\n";
        ...
        fileContents += "@SG_ML_BLANKLINE\n";
        fileContents += "@SG_ML_HEADER_sg_classlist List of classes in this module\n";
        SGLVector<SGXString> classList;
        for(SGLUnorderedMap<SGXString, SGDMCppClass, SGLEqualsTo<SGXString>, SGLHash<SGXString>>::ConstIterator i = SGDMCppClass::allClasses.constBegin(); i != SGDMCppClass::allClasses.constEnd(); i++){
            if(i.value().moduleName == moduleName){classList.pushBack(i.value().className);}
        }
        SGLSort<SGXString, SGLLesserThan<SGXString>>::sort(classList.pointerToData(0), classList.pointerToData(classList.length()));
        for(int i=0; i<classList.length(); i++){
            fileContents += SGXString("&#9;") + SGDMSgmlGeneration::createLink(classList.at(i), SGXString("../cppclass/") + classList.at(i).getLowerLanguageAware().replace(SGXChar(':'), SGXChar('_'))) + "\n";
        }
        
        const SGXString targetFolder = SGXFileSystem::joinFilePaths(SGDMDocumentationParsing::sourcePath + "SGML_source", "modules");
        SGXFile::writeAllText(SGXFileSystem::joinFilePaths(targetFolder, moduleName.getLowerLanguageAware() + ".sgml"), fileContents.replace(SGXChar('<'), SGXChar(0x2039)).replace(SGXChar('>'), SGXChar(0x203A)));
    ...
    fileContents += "@SG_ML_HEADER_sg_detaileddescription Detailed Description\n";
    fileContents += currentClass.detailedDescription + "\n";
    fileContents += "@SG_ML_BLANKLINE\n";
    fileContents += "@SG_ML_HEADER_sg_implementationdetails Implementation Details\n";
